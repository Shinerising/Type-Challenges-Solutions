...
// ============= Your Code Here =============
type Combination<T extends string[], U = T[number], A = U> = U extends string ? U | `${U}${Combination<[], Exclude<A, U>>}` : never
type Convert<T extends string> = T extends `${infer H}${infer R}` ? [H, ...Convert<R>] : []
type AllCombinations<T extends string> = Combination<Convert<T>> | ''
